...
module Jenkins2
	module IntegrationTest
		class CliPluginsTest < Minitest::Test
			PLUGINS = %w{ant create-fingerprint}
			...
			def test_install_by_source
				assert_equal true, Jenkins2::CLI::InstallPlugin.new( @@opts.merge source:
					'http://mirrors.jenkins-ci.org/plugins/chucknorris/0.9/chucknorris.hpi', name: 'chucknorris.hpi' ).call
				Jenkins2::Util.wait( max_wait_minutes: 1 ) do
			...
			def test_uninstall_plugin
				...
				assert_equal true, Jenkins2::CLI::UninstallPlugin.new( @@opts.merge name: 'create-fingerprint' ).call
